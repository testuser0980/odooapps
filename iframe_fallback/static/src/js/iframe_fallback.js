...
import {onMounted, onWillStart, onWillUnmount, useEffect, useExternalListener, useRef, useState} from "@odoo/owl";
...
patch(WebsitePreview.prototype, {
    setup() {
        ...
        this.iframeFallbackUrl = window.location.origin;
        ...
        onWillStart(async () => {
            ...
            if (this.websiteDomain && !wUtils.isHTTPSorNakedDomainRedirection(this.websiteDomain, window.location.origin)) {
                ...
                this.initialUrl = window.location.origin + `/website/force/${encodeURIComponent(this.websiteId)}?path=${encodedPath}`;
            }
        });
